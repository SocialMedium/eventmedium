...
async function insertEvent(ex, fallbackUrl, runId, stats) {
  ...
  var res = await dbRun(
    `INSERT INTO events (name, description, event_date, city, country, event_type, themes, slug, source_url, expected_attendees, image_url)
    ...
    [ex.name, ex.description || null, ex.event_date || null, ex.city || null, ex.country || null,
     ...
     ex.website || fallbackUrl, ex.expected_attendees || null, ex.image_url || null]
  );
...
async function runDiscoverySlice(runId, deadline, stats) {
  ...
  for (var q = 0; q < batch.length; q++) {
    ...
    try {
      ...
      stats.queries++;
      if (results.length) stats.queries_with_results = (stats.queries_with_results || 0) + 1;
    } catch (e) {
      stats.search_errors++;
      if (!stats.first_search_error) stats.first_search_error = String(e.message || e).slice(0, 200);
      console.error('[Ingest] SEARCH PROVIDER FAILURE (' + stats.search_errors + '): ' + e.message);
      // A dead provider fails every query — bail out early rather than burning
      // the whole slice, and let the run be marked failed below.
      if (stats.search_errors >= 5 && stats.queries === 0) {
        throw new Error('search provider is down — aborted after ' + stats.search_errors +
          ' consecutive failures: ' + stats.first_search_error);
      }
      continue;
    }
...
// ── Image backfill: grab og:image/favicon for events that predate image_url ──
// Cheap pass (plain fetch + meta parse, no Claude) capped per run so the whole
// catalogue fills in over a few nights.
async function runImageBackfill(deadline, stats) {
  var { fetchPage, extractImageFromHtml } = require('./event-harvester');
  var rows = await pool.query(
    `SELECT id, source_url FROM events
     WHERE image_url IS NULL AND source_url IS NOT NULL AND source_url != ''
       AND (event_date IS NULL OR event_date >= CURRENT_DATE)
     ORDER BY event_date ASC NULLS LAST LIMIT 25`
  );
  if (!rows.rows.length) return;
  console.log('[Ingest] Image backfill: ' + rows.rows.length + ' events');
  stats.images = 0;
  ...
  for (var i = 0; i < rows.rows.length; i++) {
    ...
    var ev = rows.rows[i];
    try {
      var html = await fetchPage(ev.source_url);
      var img = extractImageFromHtml(html, ev.source_url);
      if (img) {
        await dbRun('UPDATE events SET image_url = $1 WHERE id = $2', [img, ev.id]);
        stats.images++;
      } else {
        // Mark as tried so we don't re-fetch the same imageless page nightly
        await dbRun("UPDATE events SET image_url = '' WHERE id = $1", [ev.id]);
      }
    } catch (e) {
      // Unreachable page: mark tried, favicon fallback still works client-side
      await dbRun("UPDATE events SET image_url = '' WHERE id = $1", [ev.id]).catch(function() {});
    }
    await sleep(500);
  }
  console.log('[Ingest] Image backfill: ' + stats.images + ' images captured');
}
...
async function runIngestionCycle(opts) {
  ...
  try {
    ...
    // Representative images for events that don't have one yet
    await runImageBackfill(deadline, stats).catch(function(e) {
      console.error('[Ingest] Image backfill error:', e.message);
    });

    // A run that issued queries but got zero results from every one of them is a
    // provider failure, not a quiet night. Mark it degraded so it is visible in
    // /api/admin/ingestion instead of reporting a misleading success.
    var ranDiscovery = kind !== 'curated';
    var noResultsAtAll = ranDiscovery && stats.queries > 0 && !stats.queries_with_results;
    var finalStatus = 'success';
    var finalError = null;
    if (stats.search_errors > 0 && stats.queries === 0) {
      finalStatus = 'failed';
      finalError = 'every search query failed: ' + (stats.first_search_error || 'unknown');
    } else if (noResultsAtAll) {
      finalStatus = 'degraded';
      finalError = stats.queries + ' queries returned zero results — search provider likely ' +
        'rate-limited, quota-exhausted, or misconfigured';
    } else if (stats.search_errors > 0) {
      finalStatus = 'degraded';
      finalError = stats.search_errors + ' of ' + (stats.queries + stats.search_errors) +
        ' queries failed: ' + (stats.first_search_error || 'unknown');
    }

    await dbRun(
      "UPDATE ingestion_runs SET status = $2, finished_at = NOW(), stats = $3, error = $4 WHERE id = $1",
      [runId, finalStatus, JSON.stringify(stats), finalError]
    );
    console.log('[Ingest] Run #' + runId + ' ' + finalStatus + ': ' + JSON.stringify(stats) +
  ...
  setTimeout(async function() {
    try {
      ...
      var last = await dbGet(
        // 'degraded' counts as "the cycle ran" — it completed, the provider was just
        // unhealthy. Only a total failure (or no run at all) should trigger catch-up,
        // otherwise a dead provider would re-run ingestion on every single boot.
        "SELECT MAX(finished_at) AS t FROM ingestion_runs WHERE status IN ('success','degraded')"
      );
