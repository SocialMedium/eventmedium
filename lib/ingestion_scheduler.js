...
async function runIngestionCycle(opts) {
  ...
  try {
    ...
    if (!process.env.SERPER_API_KEY && kind !== 'curated') {
      ...
      console.error('[Ingest] SERPER_API_KEY is not set — discovery cannot run. Falling back to curated-only.');
      kind = 'curated';
    }
...
async function getIngestionStatus() {
  ...
  return { runs: runs.rows, events: freshness, serper_configured: !!process.env.SERPER_API_KEY };
}
