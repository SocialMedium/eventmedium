...
var GOOGLE_SEARCH_CX = process.env.GOOGLE_SEARCH_CX;
...
// Raised when a search provider hard-fails (bad key, API disabled, quota, 5xx,
// network). Callers MUST count these — a provider that silently returns no
// results is indistinguishable from "the web has no matching events", which is
// how event discovery sat dead for six days reporting `success`.
function ProviderError(provider, status, body) {
  this.name = 'ProviderError';
  this.provider = provider;
  this.status = status;
  this.message = 'search provider ' + provider + ' failed: ' + status + ' ' + (body || '');
  this.stack = (new Error()).stack;
}
ProviderError.prototype = Object.create(Error.prototype);
ProviderError.prototype.constructor = ProviderError;

// Google Custom Search fallback — used when SERPER_API_KEY is absent.
// Free tier is 100 queries/day, which covers the scheduler's default 40/run.
async function googleSearchEvents(query) {
  var MAX_ATTEMPTS = 3;
  for (var attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    try {
      var url = 'https://www.googleapis.com/customsearch/v1' +
        '?key=' + encodeURIComponent(GOOGLE_SEARCH_API_KEY) +
        '&cx=' + encodeURIComponent(GOOGLE_SEARCH_CX) +
      ...
      if (resp.status === 429 || resp.status >= 500) {
        ...
        // Provider is refusing/failing — THROW so the caller counts it as a
        // search error. Returning [] here is what let a dead provider report
        // `search_errors: 0` while adding nothing, night after night.
        throw new ProviderError('google_cse', resp.status, await resp.text());
      }
      if (!resp.ok) {
        // 401/403 (key invalid, API not enabled, quota/billing) — hard failure, no retry.
        throw new ProviderError('google_cse', resp.status, (await resp.text()).slice(0, 300));
      }

      var data = await resp.json();
      return (data.items || []).map(function(r, i) {
        return { title: r.title, url: r.link, snippet: r.snippet || '', position: i + 1 };
      });
    } catch (err) {
      if (err instanceof ProviderError) throw err;
      if (attempt < MAX_ATTEMPTS) { await sleep(1500 * attempt); continue; }
      throw new ProviderError('google_cse', 'network', err.message);
    }
  }
  throw new ProviderError('google_cse', 'exhausted', 'all attempts failed');
}

async function searchEvents(query) {
  if (!SERPER_API_KEY) {
    if (GOOGLE_SEARCH_API_KEY && GOOGLE_SEARCH_CX) {
      return googleSearchEvents(query);
    }
    console.warn('No SERPER_API_KEY or Google Search credentials — skipping search for:', query);
    return [];
  }

  // Retry transient failures (429 / 5xx / network) with backoff; every
  // attempt carries a timeout so a hung request can't stall a harvest run.
  var MAX_ATTEMPTS = 3;
  for (var attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    try {
      ...
      if (resp.status === 429 || resp.status >= 500) {
        var retryBody = await resp.text();
        if (attempt < MAX_ATTEMPTS) {
          console.warn('Serper ' + resp.status + ' (attempt ' + attempt + ') — retrying:', query);
          await sleep(1500 * attempt);
          continue;
        }
        throw new ProviderError('serper', resp.status, retryBody);
      }

      if (!resp.ok) {
        throw new ProviderError('serper', resp.status, (await resp.text()).slice(0, 300));
      }

      var data = await resp.json();
      return (data.organic || []).map(function(r) {
        return {
          title: r.title,
          url: r.link,
          snippet: r.snippet || '',
      ...
      if (attempt < MAX_ATTEMPTS) {
        console.warn('Search attempt ' + attempt + ' failed for "' + query + '": ' + err.message + ' — retrying');
        await sleep(1500 * attempt);
        continue;
      }
      if (err instanceof ProviderError) throw err;
      throw new ProviderError('serper', 'network', err.message);
    }
  }
  throw new ProviderError('serper', 'exhausted', 'all attempts failed');
}
...
async function storeEvent(eventData) {
  // Reject events before 2026
  if (eventData.start_date) {
    var eventYear = new Date(eventData.start_date).getFullYear();
    if (eventYear < 2026) return { stored: false, reason: 'stale (' + eventYear + ')' };
  }
...
module.exports = { harvest, searchEvents, extractEventFromPage, ProviderError, CITIES, THEME_SEARCHES };