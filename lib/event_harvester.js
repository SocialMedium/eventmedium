  ...
  for (var attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    try {
      ...
      if (resp.status === 429 || resp.status >= 500) {
        ...
        console.error('Google Search error:', resp.status, await resp.text());
        return [];
      }
      if (!resp.ok) {
        console.error('Google Search error:', resp.status, (await resp.text()).slice(0, 200));
        return [];
      }
  ...
  for (var attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    try {
      ...
      if (resp.status === 429 || resp.status >= 500) {
        ...
        console.error('Serper error:', resp.status, retryBody);
        return [];
      }
