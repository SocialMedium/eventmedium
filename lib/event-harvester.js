...
async function extractWithClaude(pageText) {
  ...
  var systemPrompt = 'You are an event data extractor. Extract structured information from event website text.\nOnly extract events taking place in 2026 or 2027. If the event date is before 2026 or the page only shows past editions, return {"name": null}.\nRespond ONLY with valid JSON. No markdown, no explanation, no code fences.';
...
function validate(extracted, url) {
  ...
  // Normalise date and reject events before 2026
  if (extracted.event_date) {
    var d = new Date(extracted.event_date);
    if (isNaN(d.getTime())) {
      extracted.event_date = null;
    } else if (d.getFullYear() < 2026) {
      throw new Error('Event is from ' + d.getFullYear() + ' — only 2026+ events are accepted.');
    }
  }
...
// ── Representative image: og:image → twitter:image → apple-touch-icon ────────
function extractImageFromHtml(html, pageUrl) {
  try {
    ...
    var abs = new URL(candidate, pageUrl).href;
    return /^https?:\/\//.test(abs) ? abs.slice(0, 500) : null;
  } catch (e) {
    return null;
  }
}
...
async function harvestEvent(url) {
  ...
  var validated = validate(extracted, url);
  validated.image_url = extractImageFromHtml(html, url);
  return validated;
}

module.exports = { harvestEvent, fetchPage, extractImageFromHtml };
