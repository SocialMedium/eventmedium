...
var EMC2_ACTIONS = {
  canister_complete:        { amount: 100,  type: 'earn',   once: true },
...
async function getWallet(user_id) {
  // Core query uses only columns guaranteed to exist on stakeholder_profiles
  var profile = null;
  try {
    profile = await db.dbGet(
      'SELECT sp.emc2_balance, sp.emc2_lifetime_earned, sp.global_access_active, sp.founding_member, sp.founding_member_granted_at, sp.emc2_cohort, sp.emc2_cohort_number, sp.emc2_earn_multiplier, w.wallet_address, w.chain_id, w.verified FROM stakeholder_profiles sp LEFT JOIN emc2_wallets w ON w.user_id = sp.user_id WHERE sp.user_id = $1',
      [user_id]
    );
  } catch(e) {
    ...
    try {
      profile = await db.dbGet(
        'SELECT 0 as emc2_balance, 0 as emc2_lifetime_earned, false as global_access_active, false as founding_member, null as founding_member_granted_at, null as emc2_cohort, null as emc2_cohort_number, 1.0 as emc2_earn_multiplier, w.wallet_address, w.chain_id, w.verified FROM stakeholder_profiles sp LEFT JOIN emc2_wallets w ON w.user_id = sp.user_id WHERE sp.user_id = $1',
        [user_id]
      );
    } catch(e2) {
      // Even stakeholder_profiles may not have this user — return defaults
      return { emc2_balance: 0, emc2_lifetime_earned: 0, global_access_active: false, founding_member: false, founding_member_granted_at: null, emc2_cohort: null, emc2_cohort_number: null, emc2_earn_multiplier: 1.0, wallet_address: null, chain_id: null, verified: false };
    }
  }
  if (!profile) {
    // User has no stakeholder_profiles row yet — return defaults so wallet still renders
    return { emc2_balance: 0, emc2_lifetime_earned: 0, global_access_active: false, founding_member: false, founding_member_granted_at: null, emc2_cohort: null, emc2_cohort_number: null, emc2_earn_multiplier: 1.0, wallet_address: null, chain_id: null, verified: false };
  }
  return profile;
}
