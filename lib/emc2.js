...
function computeAccessSummary(profile) {
  var balance = profile.emc2_balance || 0;
  var lifetime = profile.emc2_lifetime_earned || 0;
  return {
    balance: balance,
    lifetime_earned: lifetime,
    og_member: profile.og_member || false,
    founding_member: profile.founding_member || false,
    earn_multiplier: profile.emc2_earn_multiplier || 1.0,
    cohort: profile.emc2_cohort || null,
    cohort_number: profile.emc2_cohort_number || null,
    community_match_cost: 0,
    event_match_cost: 5,
    global_match_cost: 10,
    event_matches_available: Math.floor(balance / 5),
    global_matches_available: Math.floor(balance / 10),
    low_balance: balance < 50,
    access_model: 'per_match'
  };
}

var WALLET_DEFAULTS = { emc2_balance: 0, emc2_lifetime_earned: 0, global_access_active: false, founding_member: false, founding_member_granted_at: null, og_member: false, emc2_cohort: null, emc2_cohort_number: null, emc2_earn_multiplier: 1.0, referral_code: null, wallet_address: null, chain_id: null, verified: false };

async function getWallet(user_id) {
  var profile = null;
  try {
    profile = await db.dbGet(
      'SELECT sp.emc2_balance, sp.emc2_lifetime_earned, sp.global_access_active, sp.founding_member, sp.founding_member_granted_at, sp.og_member, sp.emc2_cohort, sp.emc2_cohort_number, sp.emc2_earn_multiplier, u.referral_code, w.wallet_address, w.chain_id, w.verified FROM stakeholder_profiles sp JOIN users u ON u.id = sp.user_id LEFT JOIN emc2_wallets w ON w.user_id = sp.user_id WHERE sp.user_id = $1',
      [user_id]
    );
  } catch(e) {
    // EC³ columns may not exist yet — fall back to safe query
    try {
      profile = await db.dbGet(
        'SELECT 0 as emc2_balance, 0 as emc2_lifetime_earned, false as global_access_active, false as founding_member, null as founding_member_granted_at, false as og_member, null as emc2_cohort, null as emc2_cohort_number, 1.0 as emc2_earn_multiplier, u.referral_code, w.wallet_address, w.chain_id, w.verified FROM stakeholder_profiles sp JOIN users u ON u.id = sp.user_id LEFT JOIN emc2_wallets w ON w.user_id = sp.user_id WHERE sp.user_id = $1',
        [user_id]
      );
    } catch(e2) {
      var def = Object.assign({}, WALLET_DEFAULTS);
      def.access_summary = computeAccessSummary(def);
      return def;
    }
  }
  if (!profile) {
  ...
  profile.access_summary = computeAccessSummary(profile);
  return profile;
}
...
// Community owner: award EC³ to a member
...
module.exports = {
  ...
  computeAccessSummary: computeAccessSummary
};
